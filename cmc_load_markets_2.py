...
for item in list_slugs[:20]:
    try:
        ...
        try:
            for row in soup.findAll('table')[0].tbody.findAll('tr'):
                ...
                if '/BTC' in pair:
                    ...
                    btc_share += perc
                    crypto_share += perc
                    if i_crypto_markets < 5:
                        largest_crypto_markets += str(exchange + ' ' + pair + ', volume: ' + volume + ', market share: ' + perc_string + '%') + '\n'
                        i_crypto_markets += 1
                if '/ETH' in pair:
                    if item == 'bitcoin':
                        continue
                ...
                if '/USD' in pair and '/USDT' not in pair:
                    ...
                    fiat_share += perc
                    if i_fiat_markets < 5:
                        largest_fiat_markets += str(exchange + ' ' + pair + ', volume: ' + volume + ', market share: ' + perc_string + '%') + '\n'
                        i_fiat_markets += 1
                if '/USDT' in pair:
                    usdt_share += perc
                    fiat_share += perc
        ...
        list_market_share.append([name, crypto_share, fiat_share, btc_share, eth_share, usd_share, usdt_share, ckusd_share, eur_share, cny_share, jpy_share, krw_share])
    except: pass
df_market_shares = pd.DataFrame(list_market_share, columns=['name','BTC+ETH_pairs', 'Top_fiat_pairs', 'BTC_pairs', 'ETH_pairs', 'USD_pairs', 'USDT_pairs', 'CK.USDT_pairs', 'EUR_pairs', 'CNY_pairs', 'JPY_pairs', 'KRW_pairs'])
print(df_market_shares)
df_market_shares.to_csv('data/csv/cmc_load_markets_2.csv')